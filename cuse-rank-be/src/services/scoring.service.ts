...
export class ScoringService {
  ...
  async calculateAndStoreScores(eventId: string) {
    ...
    const normalizedWeights = this.normalizeWeightage(criteriaWeightage as Record<string, number>);

    const scores: Record<
      string,
      { finalScore: number; weightedScore: number }
    > = {};

    for (const poster of posters) {
      const posterEvaluations = evaluations.filter(
        (e) => e.poster_id.id === poster.id,
      );
      ...
      for (const evalData of posterEvaluations) {
        ...
        if (judgeScores && typeof judgeScores === 'object') {
          for (const [criterion, score] of Object.entries(judgeScores)) {
            judgeScore +=
              Number(score || 0) * (normalizedWeights[criterion] || 0);
          }
        }
      ...
      let finalScore =
        relevanceSum !== 0
          ? parseFloat((weightedSum / relevanceSum).toFixed(2))
          : 0;
    ...
    const sortedRankings = await Promise.all(
      Object.entries(scores).map(
        async ([posterId, { finalScore, weightedScore }], index) => {
          const eventRef = await this.eventsRepo.findOne({
            where: { id: eventId },
          });
          const posterRef = await this.postersRepo.findOne({
            where: { id: posterId },
          });
  ...
  async getCriteriaWeightage(eventId: string) {
    ...
    return event?.criteria && typeof event.criteria === 'object'
      ? event.criteria
      : {};
  }

  async getJudgeRelevance(judgeId, posterId) {
    const assignment = await this.judgeAssignmentsRepo.findOne({
      where: { judge_id: judgeId, poster_id: posterId },
    });
